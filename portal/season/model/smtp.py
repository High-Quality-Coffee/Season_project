...
class Model:
    ...
    def send(self, to, title="Title", message="Mail Content", template=None, **kwargs):
        sender = SMTP_SENDER
        if template is None:
            html = f"""<div style="width: 100%; min-height: 100%; background: #f5f7fb; padding-top: 48px; padding-bottom: 48px;">
    <div style="width: 80%; max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px;">
        ...
        <div
            ...
            html = fs.read(f"{template}.html").replace("{title}", title).replace("{message}", message)
            for key in kwargs:
                try:
